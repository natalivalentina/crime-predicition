...
if menu == "Model Prediction":
    with st.spinner("🔍 Loading prediction page... please wait."):
        ...
        # Predict
        def load_selected_model(algo_name):
            if algo_name == 'Random Forest':
                return load_model("model_rf.pkl")
            elif algo_name == 'XGBoost':
                return joblib.load("data/model_xgb.pkl")
            elif algo_name == 'LightGBM':
                return joblib.load("data/model_lgbm.pkl")
        
        try:
            model = load_selected_model(selected_algo)
            log_pred = model.predict(input_df)[0]
            predicted_cases = int(np.round(np.expm1(log_pred)))
        except Exception as e:
            st.error("❌ Error saat memuat model atau memprediksi")
            st.text(str(e))
            st.stop()
